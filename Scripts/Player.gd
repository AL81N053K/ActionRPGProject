...
const PlayerHurtSound = preload("res://Nodes/Instances/PlayerHurtSound.tscn")
const Projectile = preload("res://Nodes/Instances/SwishProjectile.tscn")
...
func _ready():
	...
	stats.connect("sp_signal", self, "sp_signal")
	stats.connect("no_stamina", self, "no_stamina")
	stats.connect("stamina_refilled", self, "stamina_refilled")
	stats.connect("no_health", self, "no_health")
	stats.connect("health_changed", self, "health_changed")
# warning-ignore:return_value_discarded
	GlobalSettings.connect("set_stamina_bar_style", self, "set_stamina_bar_style")
...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("roll") and out_of_stamina == false:
		if state != ROLL:
...
func no_health():
	...
	while slot < 29 and die == true:
		item = PlayerInventory.at_slot(slot)
...
func _on_HurtBox_area_entered(area):
	if area.damage > 0 and die == false:
		...
		if stats.temp_health > 0:
			...
			if stats.temp_health < 0: 
				damage = -(stats.temp_health)
				stats.temp_health = 0
			else:
				damage = 0
		if damage > stats.max_health:
			stats.set_health(stats.health - clamp(damage - stats.defence, 0, damage) + 1)
		else: 
			stats.set_health(stats.health - clamp(damage - stats.defence, 0, damage))
		hurtBox.start_invincibility(1.3)
		hurtBox.create_hit_effect()
		var playerHurtSound = PlayerHurtSound.instance()
		self.add_child(playerHurtSound)
		var damage_precent = float(damage) / stats.max_health * 100
		if damage_precent <= 5:
...
func _on_GhostTimer_timeout():
	if slowmo == true:
		var ghost = preload("res://Nodes/Ghost.tscn").instance()
