...
const valid_cmd = [
	...
	["give_all_items", [], "[]"],
	["set_time_scale",[ARG_FLOAT], "[scale]"],
	["set_time",[ARG_FLOAT], "[time]"],
	["dash",[], "[]"],
]
...
func set_stat(stat, value):
	...
	match stat:
		...
		"maxtemphp","max_temp_hp","maxtemphealth","max_temp_health","tempmaxhealth","temp_max_health":
			PlayerStats.max_temp_health = value
...
func goto(place):
	var file = File.new()
	var level_path = "res://src/maps/%s.tscn" % place
	var level_path_check = file.file_exists(level_path)
	if level_path_check == true:
		var Teleport = load("res://src/prefabs/Teleporter/Teleport.tscn")
...
func give_item(id, quantity):
	...
	while (quantity > 0):
		...
		var slot = Inventories.Main.find_item(item_def, start)
		if slot == -1:
			for i in Inventories.Main.MaxStacks:
				if Inventories.Main.at_slot(i) == null: break
				if i >= 29 and Inventories.Main.at_slot(i) != null:
					spawn_item(id, quantity)
...
# warning-ignore:return_value_discarded
			...
			start += 1
		if slot != -1:
			var stackSize = Inventories.Main.at_slot(start).stackSize
...
func spawn_projectile(pos_x, pos_y, time_stamp, health, damage, speed):
	var Projectile = load("res://src/prefabs/Enemies/DummyProjectile.tscn")
...
func set_time_scale(scale):
	WorldTime.TIME_SCALE = float(scale)
	return str("Time cycle changed to ",WorldTime.TIME_SCALE, "s")

func set_time(time):
	time = float(time)
	var a_time = time * 2
	if a_time > 24: time = fmod(a_time, 24)
	WorldTime.set_time(a_time)
	return str("Time set to %s") % time

func dash():
	PlayerStats.dash = not PlayerStats.dash
