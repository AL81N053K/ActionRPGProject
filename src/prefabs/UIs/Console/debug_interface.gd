...
func _ready():
	if EventBus.connect("fps_display", self, "_display_changed") == OK: pass
	if EventBus.connect("fps_cap", self, "_fps_check") == OK: pass
...
func _process(_delta):
	...
	ram.text = str(_ram_usage, " MB RAM ", _ram_peak ," MB RAM (peak)")
